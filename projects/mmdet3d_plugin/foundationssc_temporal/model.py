...
class FoundationSSCTemporalModel(FoundationSSCImageModel):
    ...
    def _forward_occupancy(self, img_inputs, img_metas):
        ...
        # todo 后续将 forward 中的 history_features 传入此处，构建历史体素并按 temporal_metas 对齐。
    ...
    def forward(self, return_loss=False, history_img_inputs=None,
        ...
        history_features = (self.extract_history_image_features(history_img_inputs, history_img_metas)
                            if history_img_inputs is not None else [])
        ...
        result = super().forward(return_loss=return_loss, **kwargs)
        #* 原始输出模式附带历史特征便于验证；损失字典及正式评估协议不增加额外字段。
        if kwargs.get('return_outputs', False) or (not return_loss and kwargs.get('gt_occ') is None):
