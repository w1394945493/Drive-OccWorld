...
class Feature(nn.Module):
    def __init__(self, args):
        ...
        model = timm.create_model(
            ...
            pretrained=False,
            features_only=False,
            #* 不加载独立 EdgeNeXt 权重；由外层完整 stereo checkpoint 统一恢复。
        )
    ...
    def forward(self, x):
        ...
        # depthanythingv2-dinov2 features
        dinov2_feat = [
            (
                F.interpolate(
                    x.reshape(
                        B, H_resize // self.patch_size, W_resize // self.patch_size, -1
                    )
                    .permute(0, 3, 1, 2)
                    .contiguous(),  # dionv2特征reshape
                    size=(H // 16, W // 16),
                    mode="bilinear",
                    align_corners=True,
                ),
            ...
            for x, cls_token in output["features"]
        ]
        ...
        x4 = self.conv4(x4)
        return [x4, x8, x16, x32], vit_feat, dinov2_feat
