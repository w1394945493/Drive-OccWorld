...
class dformerv2(nn.Module):
    ...
    def forward(self, x, x_e):  # rgb, depth
        # rgb input
        # x = self.patch_embed(x) # [b, 3, h, w] -> [b, h/4, w/4, 64]
        x = x.permute(0, 2, 3, 1).contiguous()  # [b, 3, h, w] -> [b, h, w, 3]
        # depth input
        x_e = x_e[:, 0, :, :].unsqueeze(
            1
        )  # 在dataset中depth扩展成了三个通道，现在转成一个通道
