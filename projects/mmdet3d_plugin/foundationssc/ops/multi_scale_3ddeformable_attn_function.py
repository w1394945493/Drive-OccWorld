...
#* 原 FoundationSSC 前后向封装不变，仅将 dfa3D 改为本地相对导入。
from .dfa3D import ext_loader

ext_module = ext_loader.load_ext(
    "_ext",
    [
        "wms_deform_attn_backward",
        "wms_deform_attn_forward",
        "ms_depth_score_sample_forward",
        "ms_depth_score_sample_backward",
    ],
)
...
class MultiScaleDepthScoreSampleFunction_fp16(Function):
    ...
    def backward(ctx, grad_output: torch.Tensor) -> tuple:
        ...
        return grad_value, None, None, grad_sampling_loc, None
...
class MultiScale3DDeformableAttnFunction_fp32(Function):
    ...
    @once_differentiable
    def backward(
        ctx, grad_output: torch.Tensor, grad_depth_score_: torch.Tensor
    ) -> tuple:
        """GPU version of backward function.
        ...
        # ToDo: backward do not consider the bilinear weights currently.
        grad_value = torch.zeros_like(value)
        grad_sampling_loc_ = torch.zeros(
